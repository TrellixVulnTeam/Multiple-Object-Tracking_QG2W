...
def convert_to_tfrecords(img_path, seg_path, out_path, split, encoded=True):
    ...
    for im_file_name in img_list:
        print(im_file_name)
        im_data = tf.gfile.GFile(im_file_name, 'rb').read()
        ...
        label_data = tf.gfile.GFile(label_file_name, 'rb').read()

        im = np.asarray(Image.open(BytesIO(im_data)))
        if len(im.shape) < 3:
            continue
        rows, cols, depth = im.shape[0], im.shape[1], im.shape[2]
        if not encoded:
            im_data = np.asarray(Image.open(BytesIO(im_data))).tostring()

            label_data = np.asarray(Image.open(BytesIO(label_data)))
