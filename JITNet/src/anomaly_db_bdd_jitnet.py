...
tf.app.flags.DEFINE_integer(
    'num_scans', 25,
    'Max number of scans on the sequence.')

tf.app.flags.DEFINE_integer(
    'min_stride', 8,
    'Minimum stride for the online updates.')
...
tf.app.flags.DEFINE_boolean(
    'train_multi_res', False,
    'Train a multi res architecture.')
...
tf.app.flags.DEFINE_integer(
    'sequence_limit', 1,
    'Number of sequences loaded into memory.')
...
def resseg_model(input, height, width, scale, weight_decay,
                 ...
                 dual_model=False, multi_res=False):
    model = res_seg
    if dual_model:
        model = res_seg_dual
    if multi_res:
        model = res_seg_multi_res
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.checkpoint_path is not None and \
            not os.path.exists(FLAGS.checkpoint_path):
    ...
    def clone_fn(batch_image, labels, label_weights):
        ...
        logits, end_points = resseg_model(batch_image, FLAGS.height,
                                          ...
                                          multi_res=FLAGS.train_multi_res,
        ...
        #probs = tf.nn.softmax(logits)
        probs = tf.one_hot(tf.argmax(logits, axis=3), num_classes)
        fp = tf.reduce_sum(probs * (1. - one_hot_labels), axis=[1, 2])
        tp = tf.reduce_sum(probs * one_hot_labels, axis=[1, 2])
        fn = tf.reduce_sum((1. - probs) * one_hot_labels, axis=[1, 2])
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        s = FLAGS.min_stride
        while s <= 64:
            training_strides.append(s)
            s = s * 2
        print(training_strides)
        ...
        augmentation_list = []
        augmentation_list.append(iaa.Fliplr(1.0))
        augmentation_list.append(iaa.GaussianBlur(sigma=1.5))
        augmentation_list.append(iaa.MedianBlur(k=3))
        augmentation_list.append(iaa.BilateralBlur(d=3))
        augmentation_list.append(iaa.ContrastNormalization(0.5))
        augmentation_list.append(iaa.ContrastNormalization(1.5))
        augmentation_list.append(iaa.SaltAndPepper(0.05))
        augmentation_list.append(iaa.AdditiveGaussianNoise(scale=0.1*255, per_channel=True))
        augmentation_list.append(iaa.Dropout(0.01))
        augmentation_list.append(iaa.Affine(rotate=-15))
        augmentation_list.append(iaa.Affine(rotate=15))
        ...
        while curr_scan < FLAGS.num_scans:
            ...
            for frame, boxes, classes, scores, \
                ...
                if tracking_object:
                    # Find best matching segmentation
                    start = time.time()
                    ...
                    picked_idx, picked_iou, picked_area = \
                        mask_rcnn_get_best_instance_match(boxes, masks,
                                                          scores,
                                                          preds > 0,
                                                          (FLAGS.height, FLAGS.width),
                                                          match_threshold=match_threshold)

                    # Drop object if no match is found
                    if picked_idx < 0:
                        print('Lost Track')
                        tracking_object = False
                        # Check for discrepancies
                        if picked_iou < miss_threshold:
                            print('Anomaly')
                            discrepancy = True
                            anomaly_id = anomaly_id + 1
                        else:
                            # Clear out replay buffer
                            replay_buffer.clear()
                            # Move to next frame
                        ...
                        tracks[track_id].append((curr_frame,
                                                 boxes[picked_idx],
                                                 masks[picked_idx],
                                                 classes[picked_idx],
                                                 scores[picked_idx]))

                        # Stop track if the box becomes too small
                        area_threshold = 0.05 * FLAGS.height * 0.05 * FLAGS.width
                        if picked_area < area_threshold:
                            print('Stopped track')
                            tracking_object = False
                            # Clear out replay buffer
                            replay_buffer.clear()
                    ...
                    # Pick an object to track
                    high_conf_idx = np.where((scores > pick_threshold) > 0)

                    # Only track objects previously not tracked
                    untracked_idx = []
                    ...
                    if len(untracked_idx) > 0:
                        # Pick a candidate to attempt tracking
                        choice = random.randint(0, len(untracked_idx) - 1)
                        picked_idx = untracked_idx[choice]
                        track_info = (curr_frame, boxes[picked_idx],
                                      masks[picked_idx], classes[picked_idx],
                                      scores[picked_idx])
                    else:
                ...
                filtered_scores = np.array(scores[picked_idx])
                filtered_boxes = np.array(boxes[picked_idx])
                filtered_masks = np.array(masks[picked_idx])
                filtered_classes = np.array(classes[picked_idx])

                filtered_scores = np.expand_dims(np.expand_dims(filtered_scores, 0), 0)
                filtered_boxes = np.expand_dims(np.expand_dims(filtered_boxes, 0), 0)
                filtered_masks = np.expand_dims(np.expand_dims(filtered_masks, 0), 0)
                filtered_classes = np.expand_dims(np.expand_dims(filtered_classes, 0), 0)

                # Get attention mask for the picked object
                ...
                all_vals, _ = batch_segmentation_masks(1,
                                                    (FLAGS.height, FLAGS.width),
                                                    boxes,
                                                    classes,
                                                    masks,
                ...
                curr_updates = 0
                while (curr_updates < FLAGS.max_updates) and not discrepancy:
                    assert(picked_idx >= 0)
                    ...
                    if min(cls_scores[0]) > FLAGS.accuracy_threshold:
                        break

                end = time.time()

                if min(cls_scores[0]) > FLAGS.accuracy_threshold:
                    if not tracking_object and not discrepancy:
                        assert(picked_idx >= 0)
                        tracking_object = True

                        # Create a new track
                        print('Starting New Track')
                        assert(picked_idx not in tracked_detections[curr_frame])
                        tracked_detections[curr_frame].append(picked_idx)
                        track_id = track_id + 1
                        tracks[track_id] = []
                    ...
                    if tracking_object and picked_idx > 0:
                        print('Unable to adapt to tracked object')
                    if not tracking_object and picked_idx > 0:
                        print('Unable to start new track')
                    curr_stride_idx = max(curr_stride_idx - 1, 0)

                if discrepancy and FLAGS.anomaly_out_dir:
                    vis_preds = visualize_masks(preds, in_images.shape[0],
                                                (preds.shape[1], preds.shape[2], 3),
                                                num_classes=num_classes)
                    vis_labels = visualize_masks(all_vals, in_images.shape[0],
                                                 (all_vals.shape[1], all_vals.shape[2], 3),
                                                 num_classes=num_classes)
...
def visualize_tracks(video_in_path, tracks,
    ...
    for t in tracks.keys():
        for f, box, _, _, _ in tracks[t]:
            if f not in frame_to_tracks:
                frame_to_tracks[f] = []
            frame_to_tracks[f].append(box)
    ...
    for f in s:
        ...
        if curr_frame in frame_to_tracks:
            for b in frame_to_tracks[curr_frame]:
                x1 = int(b[0] * height)
                y1 = int(b[1] * width)
                x2 = int(b[2] * height)
                y2 = int(b[3] * width)
                cv2.rectangle(f, (y1, x1), (y2, x2),
                              (0, 255, 0), 4)
