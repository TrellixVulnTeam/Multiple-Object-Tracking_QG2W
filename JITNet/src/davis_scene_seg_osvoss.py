...
tf.app.flags.DEFINE_string(
    'train_img_path', None, 'Directory containing training data.')

tf.app.flags.DEFINE_string(
    'train_seg_path', None, 'Directory containing the segmentations.')
...
tf.app.flags.DEFINE_boolean(
    'augment_data', False,
    'Data augmentation using the initial frame.')

tf.app.flags.DEFINE_string('sequence',
                           "", 'sequence name')
...
PALETTE_PATH='/home/stevenzc3/git/JITNet/davis-2017/data/palette.txt'
...
def imwrite_indexed(filename, array):
    """Save segmentation mask in DAVIS format."""
...
def _get_init_fn():
    ...
    OVERRIDE='/mnt/disks/tensorflow-disk/jitnet_v2_1x_0.5w_ytvos_pretrained/model.ckpt-75000'
    print(OVERRIDE)
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
        imgs = ImageSequence(FLAGS.train_img_path, '*.jpg')
        segs = ImageSequence(FLAGS.train_seg_path, '*.png')
    ...
    def clone_fn(batch_image, labels):
        ...
        if FLAGS.focal_loss:
            ...
            cross_entropy = tf.losses.sparse_softmax_cross_entropy(labels,
                                                                   logits,
                                                                   scope='xentropy')
            tf.losses.add_loss(10 * cross_entropy)
        ...
        return end_points, labels, logits, entropy, class_scores

    # Gather initial summaries.
    summaries = set(tf.get_collection(tf.GraphKeys.SUMMARIES))

    clones = model_deploy.create_clones(deploy_config, clone_fn, [batch_image, labels])
    ...
    end_points, labels_tensor, logits_tensor, \
            entropy, class_scores = clones[0].outputs
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        OSVOSS_PATH = '/home/stevenzc3/osvoss/' + FLAGS.sequence
        
        for img_tp, label_tp in zip(imgs, segs):
            frame, _ = img_tp
            labels_vals, _ = label_tp
            labels_vals = (np.amin(labels_vals, axis=2) > 0).astype(np.uint8)

            # replace labels_vals with osvoss frame if curr_frame > 0
            if curr_frame > 0:
                print('made it here')
                lbl_temp = imread_indexed(os.path.join(OSVOSS_PATH, '{:05d}.png'.format(curr_frame)))
                lbl_temp = cv2.resize(lbl_temp.astype(np.uint8), dsize=(FLAGS.width, FLAGS.height), interpolation=cv2.INTER_NEAREST)
                labels_vals = (lbl_temp > 0).astype(np.uint8)
                assert(labels_vals.shape[0] == FLAGS.height)
            ...
            labels_vals = np.expand_dims(labels_vals, axis=0)
            ...
            if (curr_frame % train_stride == 0 or
                ...
                if FLAGS.augment_data:
                    segmap = ia.SegmentationMapOnImage(labels_vals[0], shape=frame[0].shape,
                                                       nb_classes=1+1)
                    for aug in augmentation_list:
                        frame_aug = aug.augment_image(frame[0])
                        labels_aug = aug.augment_segmentation_maps([segmap])[0]
                        labels_aug = labels_aug.get_arr_int()

                        frame_aug = np.expand_dims(frame_aug, axis=0)
                        labels_aug = np.expand_dims(labels_aug, axis=0)

                        replay_buffer[replay_buffer_idx] = frame_aug, labels_aug
                        replay_buffer_idx = (replay_buffer_idx + 1)%FLAGS.replay_buffer_size

                buffer_idxs = list(replay_buffer.keys())
                curr_updates = 0
                ...
                while (curr_updates < FLAGS.max_updates):
                    ...
                    for r in range(FLAGS.replay_samples):
                        ...
                        rframe, rlabels_vals = replay_buffer[idx]
                ...
                if stats_path:
                    ...
                    # save prediction
                    array = preds[0].astype(np.uint8)
                    imwrite_indexed(os.path.join(stats_path, '{:05d}.png'.format(curr_frame)), array)

                if vid_out:
                    vis_preds = preds[0]
                    vis_labels = labels_vals[0]
                    vis_frame = frame[0]

                    probs_image = np.full(vis_frame.shape, 255) * np.expand_dims(1-prob_vals[0], axis=2)
                    probs_image = probs_image.astype(np.uint8)

                    vis_labels = (np.full(vis_frame.shape, 255) * np.expand_dims(vis_labels, axis=2)).astype(np.uint8)
                    vis_preds = (np.full(vis_frame.shape, 255) * np.expand_dims(vis_preds > 0, axis=2)).astype(np.uint8)
