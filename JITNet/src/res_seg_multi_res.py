...
def ressep_backseg(inputs,
    ...
    if (scale < 1.0):
        ...
        scaled_inputs = tf.image.resize_images(inputs, rescale_dims)
    else:
    ...
    with tf.variable_scope(scope, 'ressep_map_seg', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d], outputs_collections=end_points_collection):
            ...
            with slim.arg_scope([slim.batch_norm], is_training=train_bnorm):
                ...
                rescale_dims = [int(0.5 * scale * h), int(0.5 * scale * w)]
                ...
                net = resnet_utils.conv2d_same(net, channels, 3, stride=2, scope='conv1_low_res')
                shp = net.get_shape().as_list()
                channels = max(int(64 * filter_depth_multiplier), min_enc_filters)
                net = resnet_utils.conv2d_same(net, channels, 3, stride=2, scope='conv2_low_res')

                frame_encoder_low_res_outs = []
                frame_encoder_low_res_sizes = []

                with tf.variable_scope('frame_encoder_low_res', values = [net]):
                    for b in range(len(frame_encoder_filter_sizes)):
                        shp = net.get_shape().as_list()
                        frame_encoder_low_res_sizes.append([shp[1], shp[2]])

                        filter_depth = 2 * frame_encoder_filter_sizes[b]
                        num_units = 2 * frame_encoder_num_units[b]
                        stride = 1
                        for u in range(num_units - 1):
                            if use_seperable_convolution:
                                net = basic_sep(net, filter_depth, stride, use_batch_norm=use_batch_norm)
                        ...
                        frame_encoder_low_res_outs.append(net)
                ...
                with tf.variable_scope('background_decoder', values = [net]):
                    ...
                    for b in range(num_decoder_blocks):
                        ...
                        frame_stream_low_res = frame_encoder_low_res_outs[num_decoder_blocks - b - 1]

                        shp = frame_stream.get_shape().as_list()
                        upsampled = tf.image.resize_images(frame_stream_low_res, [shp[1], shp[2]])

                        frame_stream = tf.concat([frame_stream, upsampled], axis=3)

                        if net is None:
                            net = frame_stream
                        else:
                            if encoder_decoder_skips:
        ...
        end_points['scaled_inputs'] = scaled_inputs
...
def ressep_factory(inputs,
    ...
    return ressep_backseg(inputs,
                          ...
                          encoder_decoder_skips = encoder_decoder_skips,
                          scale = scale)
