...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.stats_path is not None and os.path.exists(FLAGS.stats_path):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for frame, boxes, classes, scores, masks, num_objects, frame_id in input_streams:
            ...
            if tracking_object:
                ...
                picked_idx, picked_iou = mask_rcnn_get_best_instance_match(boxes, masks,
                                                                         scores,
                                                                         preds > 0,
                ...
                if picked_idx < 0:
                    ...
                    if picked_iou < 0.1 and prev_iou > 0.5:
                        print('Discrepancy')
                        discrepancy = True
                    else:
                ...
                high_conf_idx = scores > pick_threshold

                filtered_scores = np.array(scores[high_conf_idx])
                filtered_boxes = np.array(boxes[high_conf_idx])
                filtered_masks = np.array(masks[high_conf_idx])
                filtered_classes = np.array(classes[high_conf_idx])

                if filtered_scores.shape[0] > 0:
                    picked_idx = random.randint(0, filtered_scores.shape[0] - 1)
            ...
            filtered_scores = filtered_scores[picked_idx]
            filtered_boxes = filtered_boxes[picked_idx]
            filtered_masks = filtered_masks[picked_idx]
            filtered_classes = filtered_classes[picked_idx]
            ...
            training_str = "training: {0:.5f}s minIoU: {1:.5f}".format(end - start, min(cls_scores[0]))
            ...
            if stats_path:
                ...
                update_stats(labels_vals[0], preds[0], class_tp, class_fp, class_fn,
                             ...
                             entropy_vals, curr_frame, True, curr_updates,
                             per_frame_stats)
                end = time.time()
