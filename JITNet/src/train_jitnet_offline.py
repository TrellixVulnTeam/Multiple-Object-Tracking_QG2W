"""Training script for video distillation"""
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    for s in sequence_to_video_list[FLAGS.sequence]:
        ...
        tfrecord_name = s[1].split('.')[0] + '.tfrecords'
        print(os.path.join(FLAGS.tfrecord_dir, tfrecord_name))
        assert(os.path.exists(os.path.join(FLAGS.tfrecord_dir, tfrecord_name)))

        tfrecords_paths.append(os.path.join(FLAGS.tfrecord_dir, tfrecord_name))
        num_sequences = num_sequences + 1
        if num_sequences >= FLAGS.sequence_limit:
            break

    class_groups = video_distillation.sequence_to_class_groups_stable[FLAGS.sequence]

    print(video_files)
    print(tfrecords_paths)
    print(class_groups)

    class_groups = [ [video_distillation.detectron_classes.index(c) for c in g] \
                     for g in class_groups ]
    max_group_len = max([ len(g) for g in class_groups])
    class_groups = [ g + [g[-1]] * (max_group_len - len(g)) for g in class_groups ]
    num_classes = len(class_groups) + 1

    with tf.device(deploy_config.inputs_device()):
        iterator = get_dataset(tfrecords_paths,
                               batch_size = FLAGS.batch_size,
                               buffer_size = 250 * FLAGS.num_clones,
                               num_threads = 4 * FLAGS.num_clones,
                               height = FLAGS.height,
                               width = FLAGS.width)
        ...
        batch_scores = vals[2]
        batch_masks = vals[3]
        batch_classes = vals[4]
        batch_num_objects = vals[5]

        labels, label_weights_in = tf.py_func(batch_segmentation_masks,
                                           [FLAGS.batch_size,
                                           (FLAGS.height, FLAGS.width, 3),
                                           batch_boxes, batch_classes, batch_masks,
                                           batch_scores, batch_num_objects, True,
                                           np.array(class_groups)],
                                           (tf.uint8, tf.bool))

        label_weights_in = tf.cast(label_weights_in, tf.int32)
    ...
    def clone_fn(batch_image, labels, label_weights):
        ...
        batch_image.set_shape([FLAGS.batch_size, FLAGS.height, FLAGS.width, 3])
        ...
        return end_points, batch_image, labels, label_weights, logits

    # Gather initial summaries.
    summaries = set(tf.get_collection(tf.GraphKeys.SUMMARIES))

    clones = model_deploy.create_clones(deploy_config, clone_fn,
                                [batch_image, labels, label_weights])
    ...
    end_points, batch_image, labels, label_weights, logits = clones[0].outputs

    predictions = tf.argmax(logits, axis=3)

    seg_map = tf.py_func(visualize_masks, [labels, FLAGS.batch_size,
                                        (FLAGS.height, FLAGS.width, 3), num_classes],
                                        tf.uint8)
    pred_map = tf.py_func(visualize_masks, [predictions, FLAGS.batch_size,
                                        (FLAGS.height, FLAGS.width, 3), num_classes],
                                        tf.uint8)

    weight_map = tf.expand_dims(tf.cast(label_weights > 0, tf.uint8) * 255, axis=3)

    summaries.add(tf.summary.image('label_weights', weight_map))
