...
import res_seg
import res_seg_dual
import ytvos_tfrecords as ytvos
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
        iterator = ytvos.get_dataset(FLAGS.train_data_file,
    ...
    def clone_fn(iterator):
        ...
        batch_labels = tf.squeeze(tf.cast(batch_labels > 0, tf.int32))

        # Specify loss
        cross_entropy = tf.losses.sparse_softmax_cross_entropy(batch_labels,
                                                               logits,
                                                               scope='xentropy')
        tf.losses.add_loss(FLAGS.foreground_weight * cross_entropy)

        return end_points, batch_image, batch_labels, logits
    ...
    end_points, batch_image, batch_labels, logits = clones[0].outputs

    cmap = np.array([[0, 0, 0], [255, 255, 255]])
    cmap = tf.constant(cmap, tf.uint8)
    seg_map = tf.gather(cmap, batch_labels)
