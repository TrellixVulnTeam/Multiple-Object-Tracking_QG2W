...
def make_table(class_names, stats_variants, max_frames, file_name, exclude_classes,
    ...
    csv_file.write('\n')
    for v,s,name,cost_model in stats_variants:
        stats_dict = np.load(s)[0]
        if cost_model == 'osvos':
            num_classes = 2
            max_frames = max_frames - start_frame
            start_frame = 0
        else:
        ...
        num_samples, num_updates = get_adaptation_stats(stats_dict, max_frames,
                                                        start_frame=start_frame)
        total_frames = max_frames - start_frame
        if cost_model == 'jitnet':
            online_cost = total_cost_jitnet(total_frames, num_samples, num_updates,
                                     300, 7, 30)
        elif cost_model == 'flow':
            online_cost = total_cost_flow(total_frames, num_updates, 300, 250)
        teacher_cost = total_cost_jitnet(total_frames, total_frames, num_updates,
                                  300, 0, 0)
        speedup = float(teacher_cost)/online_cost

        if cost_model == 'osvos':
            cls_ious = cls_ious + [0.0] * (len(class_names) - 2)
