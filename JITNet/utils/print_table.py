...
grouping = { 'Sports(Fixed)': ['badminton1', 'squash1', 'table_tennis1', 'softball1'],
...
for group in grouping:
    column_headers = ['JITNet Offline Overfit', 'Flow Slow', 'Flow Fast', 'JITNet 0.7', 'JITNet 0.8', 'JITNet 0.9']
    mean_iou = {}
    sample_fraction = {}
    speed_up = {}
    ...
    for h in column_headers:
        if h == 'JITNet Offline Overfit' or h == 'Flow Slow' or h == 'Flow Fast':
...
print('\\arrayrulecolor{black!30}\\midrule\n \
        \multicolumn{7}{c}{Individual Sequences}\\\\')
...
for s in flat_list:
    ...
    acronym = []
    for c in classes:
        if c in class_acronyms:
            if class_acronyms[c] not in acronym:
                acronym.append(class_acronyms[c])
    ...
    for row in reader:
        if row[0] != '':
            ...
            sample_fraction = float(row[num_classes + 2])/300
            speed_up = float(row[num_classes+1])
            if row[0] == 'JITNet Offline Overfit' or row[0] == 'Flow Slow' or row[0] == 'Flow Fast':
                column_vals[row[0]] = '%.1f'%(mean_iou)
            elif row[0] == 'JITNet 0.9':
                column_vals[row[0]] = '%.1f(%.1f$\\times$, %.1f\\%%)'%(mean_iou, speed_up, sample_fraction)
            else:
                column_vals[row[0]] = '%.1f($%.1f\\times$, %.1f\\%%)'%(mean_iou, speed_up, sample_fraction)
                #column_vals[row[0]] = '%.1f($\\times$%.1f)'%(mean_iou, speed_up)
    ...
    for h in column_headers:
        if h in column_vals:
            ...
            row = row + '& ' + '-'
    print('{%s (%s)}%s\\\\'%(sequence_to_vis_name[s], ','.join(acronym), row))
